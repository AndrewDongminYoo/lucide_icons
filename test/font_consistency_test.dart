...
/// Parses the `cmap` table of `assets/lucide.ttf` and returns the set of
/// codepoints covered by its format 4 and format 12 subtables.
Set<int> _fontCodepoints() {
  final bytes = File('assets/lucide.ttf').readAsBytesSync();
  ...
  if (cmapOffset == null) {
    throw StateError('cmap table not found in assets/lucide.ttf');
  }
...
void main() {
  test('CSS, Dart and font codepoints are consistent', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Dart codepoints missing from assets/lucide.css: '
    ...
    expect(
      ...
      isEmpty,
      reason:
          'CSS codepoints not covered by assets/lucide.ttf cmap: '
